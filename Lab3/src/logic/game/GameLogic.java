...
public class GameLogic {
    ...
    public ArrayList<BaseCard> draw(int amount) {
        // TODO Implement here
		ArrayList<BaseCard> listTopCard = new ArrayList<BaseCard>();
		if (deck.size() <= amount) {
			listTopCard.addAll(deck);
			GameLogic.getInstance().getCurrentPlayerHand().addAll(listTopCard);
			return listTopCard;
		} else {
			for (int i = 0; i < amount; i++) {
				this.getCurrentPlayerHand().add(deck.get(0));
				deck.remove(0);
			}
			GameLogic.getInstance().getCurrentPlayerHand().addAll(listTopCard);
			return listTopCard;
